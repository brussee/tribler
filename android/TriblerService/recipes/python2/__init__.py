...
class Python2Recipe(TargetPythonRecipe):
    ...
    opt_depends = ['openssl', 'sqlite3', 'multiprocessing']
    ...
    def do_python_build(self, arch):
        ...
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            if 'multiprocessing' in self.ctx.recipe_build_order:
                # Include multiprocessing in python2 build
                r = Recipe.get_recipe('multiprocessing', self.ctx)
                i = ' -I' + r.get_build_dir(arch.arch)
                l = ' -L' + r.get_lib_dir(arch) #+ ' -lmultiprocessing'
                # Insert or append to env
                f = 'CPPFLAGS'
                env[f] = env[f] + i if f in env else i
                f = 'LDFLAGS'
