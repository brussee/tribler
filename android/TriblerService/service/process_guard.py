...
private_root_dir = path.realpath(path.split(os.getenv('ANDROID_PRIVATE', '/tmp/cache'))[0])
environ['PYTHON_EGG_CACHE'] = path.join(private_root_dir, 'cache', '.egg')

output_dir = path.realpath(os.getenv('PYTHON_SERVICE_ARGUMENT', ''))
ffmpeg = path.abspath('ffmpeg')
sys.argv += [
    "-c",
    ffmpeg,
    "-t",
